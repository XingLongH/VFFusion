...
class BRAUnetSystem(nn.Module):
    def __init__(self, img_size=256,depth=[3, 4, 8, 3],depths_decoder=[2,2,2,2], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.norm_up4 = norm_layer(embed_dim[2])
        if self.final_upsample == "expand_first":
            print("---final upsample expand_first---")
            self.up1 = FinalPatchExpand_X4(input_resolution=(img_size // 16, img_size // 16),
                                          dim_scale=4, dim=embed_dim[2])
            self.up2 = FinalPatchExpand_X4(input_resolution=(img_size // 8, img_size // 8),
                                          dim_scale=4, dim=embed_dim[1])
            self.up3 = FinalPatchExpand_X4(input_resolution=(img_size // 4, img_size // 4),
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            ...
            x[0] = self.up1(x[0])
            x[0] = x[0].view(B, 4 * H, 4 * W, -1)
            x[0] = x[0].permute(0, 3, 1, 2)  # B,C,H,W

            x[1] = self.up2(x[1])
            x[1] = x[1].view(B, 4 * H, 4 * W, -1)
            x[1] = x[1].permute(0, 3, 1, 2)  # B,C,H,W    

            x[2] = self.up3(x[2])
            x[2] = x[2].view(B, 4 * H, 4 * W, -1)
            x[2] = x[2].permute(0, 3, 1, 2)  # B,C,H,W

            x[3] = self.up4(x[3])
            x[3] = x[3].view(B, 4 * H, 4 * W, -1)
