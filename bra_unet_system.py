...
class BRAUnetSystem(nn.Module):
    def __init__(self, img_size=128,depth=[3, 4, 8, 3],depths_decoder=[2,2,2,2], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        stem = nn.Sequential(
            nn.Conv2d(in_chans, embed_dim[0] // 2, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
            nn.BatchNorm2d(embed_dim[0] // 2),
            nn.GELU(),
            nn.Conv2d(embed_dim[0] // 2, embed_dim[0], kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
        ...
        self.norm_up1 = norm_layer(embed_dim[0])
        self.norm_up2 = norm_layer(embed_dim[1])
        self.norm_up3 = norm_layer(embed_dim[2])
        self.norm_up4 = norm_layer(embed_dim[3])
        if self.final_upsample == "expand_first":
            print("---final upsample expand_first---")
            self.up1 = FinalPatchExpand_X4(input_resolution=(img_size // 8, img_size // 8),
                                          dim_scale=4, dim=embed_dim[3])
            self.up2 = FinalPatchExpand_X4(input_resolution=(img_size // 4, img_size // 4),
                                          dim_scale=4, dim=embed_dim[2])
            self.up3 = FinalPatchExpand_X4(input_resolution=(img_size // 2, img_size // 2),
                                          dim_scale=4, dim=embed_dim[1])
            self.up4 = FinalPatchExpand_X4(input_resolution=(img_size // 1, img_size // 1),
    ...
    def up_x4(self, x):
        ...
        if self.final_upsample == "expand_first":
            ...
            #x[0] = self.up1(x[0])
            x[0] = x[0].view(B,  H//8,  W//8, -1)
            x[0] = x[0].permute(0, 3, 1, 2)  # B,C,H,W

            #x[1] = self.up2(x[1])
            x[1] = x[1].view(B, H//4, W//4, -1)
            x[1] = x[1].permute(0, 3, 1, 2)  # B,C,H,W    

            #x[2] = self.up3(x[2])
            x[2] = x[2].view(B, H//2, W//2, -1)
            x[2] = x[2].permute(0, 3, 1, 2)  # B,C,H,W

            #x[3] = self.up4(x[3])
            x[3] = x[3].view(B, H, W, -1)
            x[3] = x[3].permute(0, 3, 1, 2)  # B,C,H,W            
            # x = self.output(x)
        return x
    def forward(self, x):
